...
{
    if(HAL_ETH_ReadPHYRegister(&heth,LAN8720_PHY_ADDRESS,reg,regval)!=HAL_OK)
        return -1;
    return 0;
}
...
{
    uint32_t temp=value;
    if(HAL_ETH_WritePHYRegister(&heth,LAN8720_PHY_ADDRESS,reg,temp)!=HAL_OK)
        return -1;
    return 0;
}
...
{
  lwip_IT_handle();
	__HAL_ETH_DMA_CLEAR_IT(&heth,ETH_DMA_IT_NIS);    //清除DMA中断标志位
	__HAL_ETH_DMA_CLEAR_IT(&heth,ETH_DMA_IT_R);      //清除DMA接收中断标志位
}
