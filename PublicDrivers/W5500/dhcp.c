...
u8*   SRC_MAC_ADDR   = EXTERN_DHCP_MAC;				/*本地MAC地址*/
u8*   GET_SN_MASK    = EXTERN_DHCP_SN;					/*从DHCP服务器获取到的子网掩码*/
u8*   GET_GW_IP      = EXTERN_DHCP_GW;					/*从DHCP服务器获取到的网关地址*/
u8*   GET_DNS_IP     = EXTERN_DHCP_DNS;				/*从DHCP服务器获取到的DNS服务器地址*/
u8*   DHCP_HOST_NAME = EXTERN_DHCP_NAME;   		/*主机名*/
u8*   GET_SIP        = EXTERN_DHCP_SIP;				/*从DHCP服务器获取到的本机IP*/
u8    DHCP_SIP[4] = {0,};											/*已发现的DNS服务器地址*/
u8    DHCP_REAL_SIP[4] = {0,}; 								/*从DHCP列表中选择使用的DHCP服务器*/
u8    OLD_SIP[4];        											/*最初获取到的IP地址*/

u8		dhcp_state       = STATE_DHCP_READY;       /*DHCP客户端状态*/
u8		dhcp_retry_count = 0;                      /*DHCP重试次数*/
u8		DHCP_timeout     = 0;                      /*DHCP超时标志位*/
...
u8 check_leasedIP(void);												/*检查获取的IP是否冲突*/
u8 parseDHCPMSG(u16 length);								/*从DHCP服务器接收消息并解析*/
...
void send_DHCP_DISCOVER(void)
{
	...
	//for(u8 i=0; i<3; i++)
	//Delay_ms(800);
	sendto(SOCK_DHCP, (u8 *)pRIPMSG, sizeof(RIP_MSG), ip, DHCP_SERVER_PORT);	
...
u8 parseDHCPMSG(u16 length)
{
	...
	len = recvfrom(SOCK_DHCP, (u8 *)pRIPMSG, length, svr_addr, &svr_port);
...
*/
u8 check_DHCP_state(u8 s) 
{
	u16 len;   																			/*定义一个表示接收数据大小变量*/
	u8  type;																				/*定义一个表示接收封包类型变量*/
 ...
 u8 check_leasedIP(void)
{
	...
	if ( sendto(SOCK_DHCP, ( uint8_t*)"CHECK_IP_CONFLICT", 17, GET_SIP, 5000))
	{
		#ifdef DHCP_DEBUG
			printf(" Conflict>\r\n");
		#endif
