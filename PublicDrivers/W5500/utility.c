...
*@return	num:返回转化后的整型数
*/
uint32 atoi32(char* str,u16 base)
{
  uint32 num = 0;
  while (*str !=0)
          num = num * base + c2d(*str++);
...
void itoa(u16 n,u8 str[5], u8 len)
{
...
*@return	成功 - 1, 失败 - 0
*/
char verify_ip_address(char* src, u8 * ip)
{
	int i;
	int tnum;
	char tsrc[50];
...
*/ 
u16 htons( 
	u16 hostshort	/**< A 16-bit number in host byte order.  */
	)
{
...
*@return 	成功为1 失败为0
*/ 
u8 check_dest_in_local(unsigned long destip)
{
	int i = 0;
	u8 * pdestip = (u8*)&destip;
	for(i =0; i < 4; i++)
	{
