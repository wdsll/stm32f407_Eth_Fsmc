...
{  
  ...
  temp=(int8_t)((reg_data[3]&0x07)-4);
  if (temp >= 0)
  {
    int32_t raw = ((int32_t)reg_data[0] << 16) | ((int32_t)reg_data[1] << 8) | reg_data[2];
    int32_t bias = DMM_Measurement_GetBias((uint8_t)temp);
    return (uint32_t)(raw - bias);
  }
  return 0U;
}
