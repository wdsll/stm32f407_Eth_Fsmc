...
*                                              内部变量
*********************************************************************************************************/
static float s_cv_duty = CV_PWM_DUTY_INIT; //CV 占空比运行态副本，初始 0.05。
static float s_cc_duty = CC_PWM_DUTY_INIT; //CC 占空比运行态副本，初始 0.02。
/*********************************************************************************************************
*                                              函数实现
*********************************************************************************************************/
...
{
	//当 APB1 预分频 ≠1 时，挂在 APB1 的定时器时钟 = 2×APB1，否则 = APB1。
	//代码用 (RCU_CFG0 & RCU_CFG0_APB1PSC) 判预分频字段非零 → 返回 apb1*2，否则 apb1。逻辑正确
...
void cv_pwm_init(uint32_t freq_hz, float duty)
{
    ...
    gpio_init(CV_PWM_PORT, GPIO_MODE_AF_PP, GPIO_OSPEED_50MHZ, CV_PWM_PIN); //复用推挽输出，正确。
    timer_deinit(CV_PWM_TIMER);

    uint32_t tclk = rcu_clock_freq_get(CK_APB2);
    uint32_t period = tclk / freq_hz; //周期寄存器 = 时钟/频率 - 1，标准算法。
...
/* ========== BUS_VOL_ADJ (TIMER2 CH0, PA6) ========== */
static float s_bus_vol_adj_duty = PWM_DUTY_SAFE;
...
void bus_vol_adj_pwm_init(uint32_t freq_hz, float duty)
{
    ...
    rcu_periph_clock_enable(BUS_VOL_ADJ_RCU);
    rcu_periph_clock_enable(RCU_TIMER2);
    rcu_periph_clock_enable(RCU_AF);
    gpio_init(BUS_VOL_ADJ_PORT, GPIO_MODE_AF_PP, GPIO_OSPEED_50MHZ,
              BUS_VOL_ADJ_PIN);
    timer_deinit(BUS_VOL_ADJ_TIMER);

    tclk = timer1_clk_hz(); /* TIMER1 and TIMER2 are both on APB1. */
    period = tclk / freq_hz;
    if (period == 0U) period = 1U;
    period -= 1U;
    ...
    timer_cfg.period = period;
    timer_cfg.clockdivision = TIMER_CKDIV_DIV1;
    timer_init(BUS_VOL_ADJ_TIMER, &timer_cfg);

    memset(&oc_cfg, 0, sizeof(oc_cfg));
    oc_cfg.outputstate = TIMER_CCX_ENABLE;
    oc_cfg.ocpolarity = TIMER_OC_POLARITY_HIGH;
    timer_channel_output_config(BUS_VOL_ADJ_TIMER, BUS_VOL_ADJ_CH, &oc_cfg);
    timer_channel_output_mode_config(BUS_VOL_ADJ_TIMER, BUS_VOL_ADJ_CH,
                                     TIMER_OC_MODE_PWM0);
    timer_channel_output_shadow_config(BUS_VOL_ADJ_TIMER, BUS_VOL_ADJ_CH,
                                       TIMER_OC_SHADOW_ENABLE);
    timer_auto_reload_shadow_enable(BUS_VOL_ADJ_TIMER);
    timer_enable(BUS_VOL_ADJ_TIMER);
    bus_vol_adj_pwm_set_duty(duty);
}

void bus_vol_adj_pwm_set_duty(float duty)
{
    uint32_t period = TIMER_CAR(BUS_VOL_ADJ_TIMER);
    s_bus_vol_adj_duty = f_clampf(duty, 0.0f, 1.0f);
    timer_channel_output_pulse_value_config(
        BUS_VOL_ADJ_TIMER, BUS_VOL_ADJ_CH,
        (uint16_t)((float)(period + 1U) * s_bus_vol_adj_duty));
}

float bus_vol_adj_pwm_get_duty(void)
{
    return s_bus_vol_adj_duty;
}