...
/**
 * @brief LLC 状态机是否处于 FAULT 状态
 */
bool llc_is_fault_state(void);

/**
 * @brief BRK 硬件故障是否已锁存（BKIN 曾经拉低过，protect_clear_fault 前一直保持）
 * 由 protect_exti 模块管理，LLC 层转发暴露供 charge 层使用
 */
int protect_fault_latched(void);

/**
 * @brief 清除 BRK 软件锁存（故障排查完毕后可调用）
 */
