...
/* TIMER0 configuration */
    ...
    /* 初始值：ARR/2，llc_pwm_set_freq 会自动重算 */
    uint32_t trigger_at = ((s_period + 1U) >> 1);    // floor(ARR/2)
...
/**
 * @brief 启用或禁用LLC PWM输出
 * 
 * @param en 使能标志，true为启用PWM输出，false为禁用PWM输出
 * 
 * @note 启用时执行以下操作：
 * 1. 强制启动LLC PWM
 * 2. 恢复PWM配置
...
{ 
   if (en) {
    ...
    llc_pwm_force_start(1);
		...
		timer_channel_output_pulse_value_config(TIMER0, LLC_PWM_CH, 0U);  // 将指定通道的CCR清零，确保下次使能时从0占空比开始
	}
...
{ 
	...
	/* 同步更新 Ch1 比较值：维持 ARR/2 触发点 */
	uint32_t trigger_at = ((s_period + 1U) >> 1);
