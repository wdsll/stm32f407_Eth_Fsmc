...
#ifndef LLC_SOFTOFF_ENABLE
#define LLC_SOFTOFF_ENABLE               (1)
#endif

#ifndef LLC_SOFTOFF_DURATION_MS
#define LLC_SOFTOFF_DURATION_MS          (30U)
#endif
...
#define LLC_HICCUP_WAIT_MS               (1000U)
#endif

#ifndef LLC_HICCUP_MAX_RETRY
#define LLC_HICCUP_MAX_RETRY             (5U)
#endif

#ifndef LLC_HICCUP_CLEAR_MS
#define LLC_HICCUP_CLEAR_MS              (5000U)
#endif
/*********************************************************************************************************
*                                              内部变量定义
*********************************************************************************************************/
...
{
    switch (reason) {
        case LLC_STOP_REASON_VOUT_OVP:
        case LLC_STOP_REASON_EXT_FAULT:
            return true;
...
static llc_stop_reason_t llc_check_stop_reason(void)
{
    /* 如后续接入硬件保护，这里直接返回 EXT_FAULT */
    /*
    if (protect_fault_active_hw() || protect_fault_latched()) {
        return LLC_STOP_REASON_EXT_FAULT;
    }
    */

    if (s_llc_rt.meas.vout_v > LLC_VOUT_OVP_V) {
        return LLC_STOP_REASON_VOUT_OVP;
    }

    if (s_llc_rt.meas.iout_a > LLC_IOUT_OCP_A) {
        return LLC_STOP_REASON_IOUT_OCP;
    }

    if (s_llc_rt.meas.vbus_v < (LLC_VBUS_MIN_START_V - LLC_VOUT_HYST_V)) {
        return LLC_STOP_REASON_VBUS_UVP;
    }

    return LLC_STOP_REASON_NONE;
}

static void llc_request_stop(llc_stop_reason_t reason)
{
    s_llc_rt.stop_reason = (uint8_t)reason;

    if (llc_stop_reason_need_latch(reason)) {
        s_llc_rt.fault_latched = 1U;
    }
...
static float llc_ctrl_step(float e)
{
     ...
     */
    float f_req = f_nom - u;   //pi环路生成的频率变化量
    float f_sat = f_clampf(f_req, f_min, f_max); //限幅之后的变化量
	...
	switch(next)
	{
	 ...
	 case ST_STOPPING:
      s_llc_rt.stopping_begin_ms = g_ms;
	   //软关就是当前的实时频率
	    s_llc_rt.softoff_start_hz = f_clampf(s_llc.f_cmd, s_llc.f_min, s_llc.f_max);
...
void llc_app_tick_1khz(void)
{
	...
	switch(s_llc_rt.app.state)
	{
        ...
        if (!enable_llc) {
          llc_request_stop(LLC_STOP_REASON_PFC_LOST);
				...
				if (!llc_precheck_ok()) {
					llc_request_stop(LLC_STOP_REASON_PRECHECK_FAIL);
					break;
				}
    ...
    case ST_RUN_ENTRY_HOLD:
		...
		case ST_LLC_RUN:
		{
			...
			if (elapsed_reached(s_llc_rt.app.entry_ms, LLC_HICCUP_CLEAR_MS)) {
					s_llc_rt.hiccup_retry_cnt = 0U;
			}
				break;
	 }
		case ST_STOPPING:
		{
		  uint32_t elapsed_ms = elapsed_since(s_llc_rt.stopping_begin_ms);
			uint32_t ramp_ms = LLC_SOFTOFF_ENABLE ? LLC_SOFTOFF_DURATION_MS : 0U;
			uint32_t total_ms = ramp_ms + LLC_STOPPING_PWM_OFF_DELAY_MS;
		#if LLC_SOFTOFF_ENABLE
				if (ramp_ms > 0U) {
						float k = (elapsed_ms >= ramp_ms) ? 1.0f : ((float)elapsed_ms / (float)ramp_ms);
						float freq = s_llc_rt.softoff_start_hz +
												 (s_llc.f_max - s_llc_rt.softoff_start_hz) * k;
						llc_set_freq(freq);
				} else {
						llc_set_freq(s_llc.f_max);
				}
			...
			if(elapsed_reached(s_llc_rt.stopping_begin_ms, stop_wait_ms))
			{
				...
				{
					#if LLC_HICCUP_ENABLE
                        llc_state_enter(ST_HICCUP_WAIT);
					#else
                        llc_state_enter(ST_IDLE);
					#endif
				}
		...
		case ST_HICCUP_WAIT:
		 ...
		 if (!elapsed_reached(s_llc_rt.hiccup_begin_ms, LLC_HICCUP_WAIT_MS)) {
					break;
			}

			if (!enable_llc || !llc_precheck_ok()) {
					s_llc_rt.hiccup_begin_ms = g_ms;
					break;
			}

			if (s_llc_rt.hiccup_retry_cnt >= LLC_HICCUP_MAX_RETRY) {
					s_llc_rt.fault_latched = 1U;
					llc_state_enter(ST_FAULT);
					break;
			}

			s_llc_rt.hiccup_retry_cnt++;
			s_llc_rt.stop_reason = LLC_STOP_REASON_NONE;
			llc_state_enter(ST_PRECHECK);
			break;
