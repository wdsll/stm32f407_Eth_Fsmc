...
#define BIT(x)                       ((uint32_t)((uint32_t)0x01U<<(x))) //这段代码的作用是在二进制中的某一位上设置为1，其他位均为0。
//这段代码的作用是将一个 32 位的无符号整数的指定位段（从第 start 位到第 end 位，包括 start 和 end ）设置为全 1，而其他位则保持不变。
#define BITS(start, end)             ((0xFFFFFFFFUL << (start)) & (0xFFFFFFFFUL >> (31U - (uint32_t)(end))))
//从一个32位的寄存器值regval中获取指定位段（从start到end）的值，并将其调整为最低位。
