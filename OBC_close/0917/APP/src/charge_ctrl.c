...
{
    charge_state_t prev = s_chg_rt.state;  /* 记录旧状态用于日志 */
    s_chg_rt.state = st;
    ...
    /* ========== 状态切换日志（非阻塞DMA串口） ========== */
    debug_printf("[CHG] %d->%d @%lu ms, Vbat=%.1fV, Vout=%.1fV, Iout=%.1fA\r\n",
                 (int)prev, (int)st,
                 (unsigned long)g_ms,
                 (double)s_chg_rt.vbat_v,
                 (double)s_chg_rt.vout_v,
                 (double)s_chg_rt.iout_a);
}
/*********************************************************************************************************
