...
#define DSPI_IP_SW_MAJOR_VERSION_C               4
...
*/
#define DSPI_IP_CTARE_DTCP_MAX_U16    (uint16) 2047u
/*==================================================================================================
*                                       LOCAL CONSTANTS
==================================================================================================*/
/*==================================================================================================
...
Dspi_Ip_StateStructureType* Dspi_Ip_apxStateStructureArray[DSPI_INSTANCE_COUNT];
...
#if (DSPI_IP_DMA_USED == STD_ON)
static void Dspi_Ip_DmaConfig(uint8 Instance);
...
static void Dspi_Ip_DmaContinueTransfer(uint8 Instance);
#if (DSPI_IP_ENABLE_DMAFASTTRANSFER_SUPPORT == STD_ON)
static void Dspi_Ip_DmaFastConfig(uint8 Instance, const Dspi_Ip_FastTransferType *FastTransferCfg, uint8 NumberOfTransfer);
static void Dspi_Ip_RxDmaTcdSGConfig(uint8 Instance, uint8 TCDSGIndex, uint8 DisHwReq);
static void Dspi_Ip_RxDmaTcdSGInit(uint8 Instance);
static void Dspi_Ip_TxDmaTcdSGConfig(uint8 Instance, uint8 TCDSGIndex, uint8 DisHwReq);
...
static void Dspi_Ip_ReadRxFifo
(
    uint16 NumberOfReads,
    uint8 Instance
);
static void Dspi_Ip_WriteTxFifo
(
    uint16 NumberOfWrites,
    uint8 Instance
);
static void Dspi_Ip_ReceiveData
(
    uint16 NumberOfReads,
    uint8 Instance
);
static void Dspi_Ip_ChannelFinished
(
    uint8 Instance, boolean ErrorFlag
);
static void Dspi_Ip_WriteCmdFifo
(
    uint16 NumberOfWrites,
    uint8 Instance
);

static void Dspi_Ip_UpdateTxRxCounter
(
    uint8 Instance,
    uint16 Length
);
#if (DSPI_IP_DEV_ERROR_DETECT == STD_ON)
static void Dspi_Ip_CheckValidParameters
(
    ...
    if (DSPI_IP_BUSY == State->Status)
    {
        ...
        else
        {
...
#endif
            {
                ...
                NumberOfWrites = (uint16)((Base->SREX) & DSPI_SREX_CMDCTR_MASK) >> DSPI_SREX_CMDCTR_SHIFT;
                NumberOfWrites = DSPI_IP_FIFO_SIZE_U16 - NumberOfWrites;
                /* Write to CMD FIFO */
                if ((NumberOfWrites != 0u) && (State->ExpectedCmdFifoWrites != 0u))
                {
                    Dspi_Ip_WriteCmdFifo(NumberOfWrites, Instance);
                }
            }
...
static void Dspi_Ip_RxDmaTcdSGConfig(uint8 Instance, uint8 TCDSGIndex, uint8 DisHwReq)
{
    ...
    {
        /* Discard data */
        DmaTcdList[0u].Value = (uint32)&Dspi_Ip_u32DiscardData;    /* dest address write*/
...
static void Dspi_Ip_DmaConfig(uint8 Instance)
{
    ...
    /* Update last command */
    State->PushrCmdLast = State->PushrCmd & (~((uint16)((DSPI_PUSHR_CONT_MASK | DSPI_PUSHR_PP_MCSC_MASK) >> 16u)));
    ...
    if(State->ExternalDevice->DeviceParams->FrameSize > 16u)
    {
        ...
        NumberDmaIterWrite may not be multiple of 2 due to it was assigned to DSPI_IP_DMA_MAX_ITER_CNT_U16
        or by user in Dspi_Ip_AsyncTransmit */
    ...
    #endif
    {
        ...
        if((boolean)TRUE == EnScatterGather)
        {
            ...
            Dspi_Ip_CmdDmaTcdSGConfig(Instance, /* Cmd Dma Channel Id */
    ...
    {
        /* Discard data */
        DmaTcdList[1u].Value = (uint32)&Dspi_Ip_u32DiscardData;    /* dest address write*/
...
* @details In the case number of data frames are greater than DSPI_IP_DMA_MAX_ITER_CNT_U16, this function will update hardware TCD for DMA channels
...
)
{
    ...
    /* Read data from RX FIFO */
    Dspi_Ip_ReadRxFifo(ActualNumberOfReads, Instance);
...
* @return void
*/
static void Dspi_Ip_ChannelFinished(uint8 Instance, boolean ErrorFlag)
{
    Dspi_Ip_StateStructureType* State = Dspi_Ip_apxStateStructureArray[Instance];
...
)
{
    ...
    {
        PushrCmd &= ~((uint16)((DSPI_PUSHR_CONT_MASK | DSPI_PUSHR_PP_MCSC_MASK) >> 16u));
    }
    Base->PUSHR.FIFO.CMD = PushrCmd;
    State->ExpectedCmdFifoWrites--;
}
...
* @return none
*/
static void Dspi_Ip_UpdateTxRxCounter(uint8 Instance, uint16 Length)
{
    Dspi_Ip_StateStructureType* State = Dspi_Ip_apxStateStructureArray[Instance];
        ...
        if (DSPI_IP_BUSY == State->Status)
        {
            ...
            else
            {
                if (State->ExpectedFifoReads != State->RxIndex)
                {
                    ...
                    Dspi_Ip_DmaContinueTransfer(Instance);
                }
                else
                {
                                     ...
                                     )
{
    ...
    Dspi_Ip_CheckValidParameters(ExternalDevice, Length);
    #endif
    ...
    else
    {
        ...
        Dspi_Ip_UpdateTxRxCounter(Instance, Length);
        /* To increase the performance, Some frames can be transferred using 1 CMD. Support maximum DSPI_IP_CTARE_DTCP_MAX_U16 frames. */
        ...
        if ((State->ExpectedCmdFifoWrites <= DSPI_IP_CTARE_DTCP_MAX_U16) && (State->ExpectedCmdFifoWrites > 1u) && (FALSE == State->KeepCs))
        {
        ...
        #if (DSPI_IP_DUAL_CLOCK_MODE == STD_ON)
        Base->MODE.CTAR[0u] = ExternalDevice->Ctar[State->ClockMode] | DSPI_CTAR_FMSZ(((uint32)State->ExternalDevice->DeviceParams->FrameSize - 1u) & 0xFu) | DSPI_CTAR_LSBFE(LsbValue);
        #else
        Base->MODE.CTAR[0u] = ExternalDevice->Ctar | DSPI_CTAR_FMSZ(((uint32)State->ExternalDevice->DeviceParams->FrameSize - 1u) & 0xFu) | DSPI_CTAR_LSBFE(LsbValue);
        #endif
        if (State->CtareDtcpSupport)
        {
            /* Use CTAR0/CTARE0 to transfer all (frames - 1) */
            Base->CTARE[0u] = DSPI_CTARE_FMSZE((((uint32)State->ExternalDevice->DeviceParams->FrameSize - 1u) >> 4u) & 0x1u) | DSPI_CTARE_DTCP((uint32)State->ExpectedCmdFifoWrites - 1u);
            Base->PUSHR.FIFO.CMD = (PushrCmd | (uint16)(DSPI_PUSHR_CTAS(0U)>>16u));
            /* Use CTAR1/CTARE1 to transfer latest frame to clear CONT bit */
            Base->CTARE[1u] = DSPI_CTARE_FMSZE((((uint32)State->ExternalDevice->DeviceParams->FrameSize - 1u) >> 4u) & 0x1u) | DSPI_CTARE_DTCP(1u);
            Base->PUSHR.FIFO.CMD = (PushrCmd | (uint16)(DSPI_PUSHR_CTAS(1U)>>16u)) & (~((uint16)((DSPI_PUSHR_CONT_MASK | DSPI_PUSHR_PP_MCSC_MASK) >> 16u)));
            /* Configure external device parameters  like: cs, clock phase, clock polarity. */
            #if (DSPI_IP_DUAL_CLOCK_MODE == STD_ON)
                Base->MODE.CTAR[1u] = ExternalDevice->Ctar[State->ClockMode] | DSPI_CTAR_FMSZ(((uint32)State->ExternalDevice->DeviceParams->FrameSize - 1u) & 0xFu) | DSPI_CTAR_LSBFE(LsbValue);
            #else
                Base->MODE.CTAR[1u] = ExternalDevice->Ctar | DSPI_CTAR_FMSZ(((uint32)State->ExternalDevice->DeviceParams->FrameSize - 1u) & 0xFu) | DSPI_CTAR_LSBFE(LsbValue);
            #endif
        }
        else
        {
            Base->CTARE[0u] = ExternalDevice->Ctare | DSPI_CTARE_FMSZE((((uint32)State->ExternalDevice->DeviceParams->FrameSize - 1u) >> 4u) & 0x1u);
        }
        ...
        {
            /* SEND CMD */
            /* if number of frames <= DSPI_IP_CTARE_DTCP_MAX_U16 and this is latest channel. All frames will be transferred using only 1 CMD which has push to TX CMD above */
                                      ...
                                      )
{
    ...
    else
    {
        ...
        {
            #if (DSPI_IP_DUAL_CLOCK_MODE == STD_ON)
                Base->MODE.CTAR[0u] = (ExternalDevice->Ctar[State->ClockMode] & (~DSPI_CTAR_SLAVE_FMSZ_MASK)) | DSPI_CTAR_SLAVE_FMSZ((uint32)State->ExternalDevice->DeviceParams->FrameSize - 1u);
            #else
                Base->MODE.CTAR[0u] = (ExternalDevice->Ctar & (~DSPI_CTAR_SLAVE_FMSZ_MASK)) | DSPI_CTAR_SLAVE_FMSZ((uint32)State->ExternalDevice->DeviceParams->FrameSize - 1u);
            #endif
        }
        #endif
        ...
        #endif
        {
            ...
            {
                case DSPI_IP_POLLING:
                    /* Disable interrupts and DMA requests. */
                    Base->RSER = 0U;
                    break;
                case DSPI_IP_INTERRUPT:
                    /* Enable interrupts. */
                    ...
                    #endif
                    {
                        ...
                        {
                            Dspi_Ip_WriteCmdFifo(NumberOfCmdWrites, Instance);
                        }
                    ...
                    else
                    {
                        ...
                        /* Write to TXFIFO*/
                        Dspi_Ip_WriteTxFifo(NumberOfWrites, Instance);
        ...
        else
        {
			...
			Dspi_Ip_DmaConfig(Instance);
            Base->RSER = DSPI_RSER_CMDFFF_RE(1) | DSPI_RSER_CMDFFF_DIRS(1) | DSPI_RSER_TFFF_RE(1) | DSPI_RSER_TFFF_DIRS(1) | DSPI_RSER_RFDF_RE(1) | DSPI_RSER_RFDF_DIRS(1);
        }
        #endif
